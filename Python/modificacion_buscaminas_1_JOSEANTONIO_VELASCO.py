...
def menu():
    ...
    if option == 1:
        #MODIFICACION 1 HA OCURRIDO AQUI BEGIN
        print("Selecciona dificultad (4,7,10): ")
        listaDificultades = [4,7,10]
        boardSize = int(input())
        while boardSize not in listaDificultades:
            print("Error dificultad no valida")
            print("Selecciona dificultad (4,7,10): ")
            boardSize = int(input())
        #MODIFICACION 1 HA OCURRIDO AQUI END
        lifes = boardSize
        mines = boardSize*2
        score = 0
        combo = 0
