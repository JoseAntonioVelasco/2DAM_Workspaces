...
def paintBoard(board,rowLetter,score,combo,lifes,mines,minesFound):
    size = len(board)
    for i in range(size):
        #HA OCURRIDO UN CAMBIO AQUI BEGIN
        print("",rowLetter.get(i), end="")
        #HA OCURRIDO UN CAMBIO AQUI END
    print()
...
def showSurrounding(hiddenBoard,visibleBoard,row,col,combo,score,minesFound,mines,lifes):
    #HA OCURRIDO UN CAMBIO AQUI han sido cambiado la row por col BEGIN
    if hiddenBoard[col][row] == "1":
        hiddenBoard[col][row] = "3"
        visibleBoard[col][row] = u'\u25c7'
    ...
    elif hiddenBoard[col][row] == "0":
        hiddenBoard[col][row] = "2"
        visibleBoard[col][row] = u'\u25c6'
    #HA OCURRIDO UN CAMBIO AQUI han sido cambiado la row por col END
        combo = 0
        score = score - 1
        lifes = lifes - 1
    ...
    while(True):  
      ...
      if minesFound == mines:
        ...
        if len(sorted_scores)==0 or (int(sorted_scores[0][2]) > score)==False:
            ...
            scoreboard.write(str(int(mines/2))+","+name+","+str(score)+"\n")
            scoreboard.close()
            secondMenu()
            return
